<template>
  ...
  <div style="flex: 1;display: flex;flex-direction: column;">
    ...
    <div id="video-container">
      <a-scrollbar id="video-scrollbar">
        <div @click="goBv(video.bvid)" class="video-box" v-for="video in renderVideoList" :key="video.id">
            ...
            <div class="video-mask">
              <div>
                <span class="video-show-count"><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24" width="16" height="16" fill="#ffffff" class="bili-video-card__stats--icon"><!--[--><path d="M12 4.99805C9.48178 4.99805 7.283 5.12616 5.73089 5.25202C4.65221 5.33949 3.81611 6.16352 3.72 7.23254C3.60607 8.4998 3.5 10.171 3.5 11.998C3.5 13.8251 3.60607 15.4963 3.72 16.76355C3.81611 17.83255 4.65221 18.6566 5.73089 18.7441C7.283 18.8699 9.48178 18.998 12 18.998C14.5185 18.998 16.7174 18.8699 18.2696 18.74405C19.3481 18.65655 20.184 17.8328 20.2801 16.76405C20.394 15.4973 20.5 13.82645 20.5 11.998C20.5 10.16965 20.394 8.49877 20.2801 7.23205C20.184 6.1633 19.3481 5.33952 18.2696 5.25205C16.7174 5.12618 14.5185 4.99805 12 4.99805zM5.60965 3.75693C7.19232 3.62859 9.43258 3.49805 12 3.49805C14.5677 3.49805 16.8081 3.62861 18.3908 3.75696C20.1881 3.90272 21.6118 5.29278 21.7741 7.09773C21.8909 8.3969 22 10.11405 22 11.998C22 13.88205 21.8909 15.5992 21.7741 16.8984C21.6118 18.7033 20.1881 20.09335 18.3908 20.23915C16.8081 20.3675 14.5677 20.498 12 20.498C9.43258 20.498 7.19232 20.3675 5.60965 20.2392C3.81206 20.0934 2.38831 18.70295 2.22603 16.8979C2.10918 15.5982 2 13.8808 2 11.998C2 10.1153 2.10918 8.39787 2.22603 7.09823C2.38831 5.29312 3.81206 3.90269 5.60965 3.75693z" fill="currentColor"></path><path d="M14.7138 10.96875C15.50765 11.4271 15.50765 12.573 14.71375 13.0313L11.5362 14.8659C10.74235 15.3242 9.75 14.7513 9.75001 13.8346L9.75001 10.1655C9.75001 9.24881 10.74235 8.67587 11.5362 9.13422L14.7138 10.96875z" fill="currentColor"></path><!--]--></svg>
                  <span class="video-show-count-text">{{ formatNumber(video.stat.view) }}</span>
                </span>
                  ...
                  <span class="video-danmu-count-text">{{ formatNumber(video.stat.danmaku) }}</span>
                </span>
              </div>
              <span class="video-size">{{ formatSecondTime(video.duration) }}</span>
            </div>
          </div>
          ...
          <div class="video-info">
            ...
            <span>{{ video.owner.name }} · {{ formatTimestamp(video.pubdate) }}</span>
          </div>
        </div>

        <a-space id="fresh-recommend">
          <a-button @click="updateVideoList" style="width: 200px;height: 60px;font-size: 24px;">刷新视频</a-button>
          <a-button @click="getRcmdVideoList" style="width: 200px;height: 60px;font-size: 24px;">加载更多</a-button>
        </a-space>
      </a-scrollbar>
    </div>
...
import {formatTimestamp, formatSecondTime} from "../utils/time_util.ts";
import {formatNumber} from "../utils/number_util.ts";

const router = useRouter();

const renderVideoList = ref([]);

const fresh_idx = ref(1);

const getRcmdVideoList = async () => {
  const response = await client.execute("index/rcmd", {
    params: {
      fresh_type: 3,
      version: 1,
      ps: 10,
      fresh_idx: fresh_idx.value,
      fresh_idx_1h: fresh_idx.value
    }
  });
  if (!response.success) {
    this.$message.warning("获取推荐视频失败，请稍后重试")
  ...
  const videoList = response.data.data.item;
  renderVideoList.value.push(...videoList);

  console.log(renderVideoList.value)
...
const updateVideoList = async () => {
  ...
  await getRcmdVideoList();
}

// 计算视频容器宽高
const calculateVideoWidth = async () => {
  const maxVideoCount = 8;
  const minVideoWidth = 240 + 16;
  const maxVideoWidth = 280 + 16;
  const videoContainerWidth = parseInt(document.getElementById("video-container").clientWidth) - 1 - 16;
  let videoWidth;
  if (videoContainerWidth / minVideoWidth < 2) {
    videoWidth = videoContainerWidth / 2;
  } else if (videoContainerWidth / maxVideoWidth > maxVideoCount) {
    videoWidth = videoContainerWidth / maxVideoCount;
  } else {
    const videoCount = parseInt(videoContainerWidth / minVideoWidth);
    videoWidth = videoContainerWidth / videoCount;
  }
  videoWidth -= 16;
  document.documentElement.style.setProperty('--index-video-width', `${videoWidth}px`);
  document.documentElement.style.setProperty('--index-video-height', `${videoWidth * 0.5625}px`);
}

let resizeObserver;

const createResizeObserver = async () => {
...
// 数据缓存
const loadData = () => {
  const index_video_list_cache = sessionStorage.getItem("index_video_list_cache");
  const index_fresh_idx_cache = sessionStorage.getItem("index_fresh_idx_cache");
  if (index_video_list_cache && index_fresh_idx_cache) {
    renderVideoList.value = JSON.parse(index_video_list_cache);
    fresh_idx.value = parseInt(index_fresh_idx_cache);
    return true;
  }
  return false;
}
...
const saveData = () => {
  sessionStorage.setItem("index_video_list_cache", JSON.stringify(renderVideoList.value));
  sessionStorage.setItem("index_fresh_idx_cache", fresh_idx.value.toString());
};
...
#video-container>.arco-scrollbar{
  overflow: hidden;
}
